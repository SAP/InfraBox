...
class Clusters(Resource):
    ...
    def get(self):
	    ...
	    SELECT cluster_name,
		    ...
		    SUM(CASE WHEN state = 'running' THEN cpu ELSE 0 END) running_cpu,
		    SUM(CASE WHEN state = 'running' THEN memory ELSE 0 END) running_memory,
		    SUM(CASE WHEN state = 'scheduled' THEN 1 ELSE 0 END) scheduled,
		    SUM(CASE WHEN state = 'scheduled' THEN cpu ELSE 0 END) scheduled_cpu,
		    SUM(CASE WHEN state = 'scheduled' THEN memory ELSE 0 END) scheduled_memory,
		    SUM(CASE WHEN state = 'queued' THEN 1 ELSE 0 END) queued,
		    SUM(CASE WHEN state = 'queued' THEN cpu ELSE 0 END) queued_cpu,
		    SUM(CASE WHEN state = 'queued' THEN memory ELSE 0 END) queued_memory
	    FROM JOB
