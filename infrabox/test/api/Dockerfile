...
RUN apk add --no-cache python py2-psycopg2 py2-pip gcc openssl libffi-dev python2-dev musl-dev py2-flask git openssl-dev py2-requests && \
    pip install bcrypt PyJWT jsonschema coverage unittest-xml-reporting urllib3 cryptography mock flask-restplus eventlet flask_socketio boto3 google-cloud-storage future codecov pycrypto && \
