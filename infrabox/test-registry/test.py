...
class InputTests(TestCase):
    def get(self, url, password='b514af82-3c4f-4bb5-b1da-a89a0ced5e6f'):
        token = encode_project_token(password, '2514af82-3c4f-4bb5-b1da-a89a0ced5e6f', 'myproject')
        auth = base64.b64encode((f'infrabox:{token}').encode()).decode()
...
class Test(TestCase):
    ...
    def _get_headers(self):
        token = 'infrabox:%s' % encode_project_token(self.token, self.project_id, 'myproject')
